...
export default function TeacherView() {
  ...
  return (
    <main className="flex-1 min-w-0 bg-white overflow-y-auto" aria-label="Teacher overview">
      <div className="px-6 py-4 border-b border-[#eaeaea] flex items-center gap-2">
        <span className="text-[11px] font-semibold tracking-widest uppercase text-[#9a9a9a]">All student canvases · full visibility</span>
      </div>
      <div className="p-6 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {students.map((s, idx) => (
          <div key={s.id} className="rounded-lg border border-[#c8c8c8] overflow-hidden">
            ...
            >
              ...
              <span className="absolute bottom-2 right-2 text-[10px] tracking-wide uppercase text-[#7a7a7a] bg-white/80 rounded px-1.5 py-0.5">
                {STATUSES[idx % STATUSES.length]}
              </span>
            </div>
            <div className="px-4 py-2.5 text-[11.5px] text-[#7a7a7a] border-t border-[#eaeaea]">
